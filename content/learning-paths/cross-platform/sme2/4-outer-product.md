...
[outer product](https://en.wikipedia.org/wiki/Outer_product) as the matrix
multiplication result can be expressed as the
...
order. This means that loading row-data from memory is somewhat efficient as the
memory as the memory system operates efficiently with contiguous data, e.g.
caches are loaded line by line, data prefetching is dead easy (just load the
data at the next address), ... This is not the case for loading column-data from
memory though.

In order to improve further the effectiveness of the matrix multiplication, it
...
transposition affects only the memory layout: ``matLeft`` will be transformed to be
in column-major order --- from a mathematical perspective, ``matleft`` is *not*
transposed.
{{% /notice %}}
...
load data to / store data from tiles efficiently, as well as instructions to operate with
/ on tiles, e.g the
...
``macc`` to ``load`` ratio, as all the tile elements are loaded to the tile, to be used
with the SME outer product instructions.
...
the matrix multiplication perform the preprocessing step correctly.
