...
with the ``IMPL`` macro defined to be ``intr`` in the ``Makefile``. Compile and
run it with:

```BASH
shrinkwrap --runtime=docker-local build SME2.yaml
shrinkwrap --runtime=docker-local run SME2.yaml --rtvar PROGRAM='${artifact:SME2_MATMUL_INTR}'
```
...
[   fvp ]
[   fvp ] SME2 Matrix Multiply fp32 *intr* example with args 125 35 70
