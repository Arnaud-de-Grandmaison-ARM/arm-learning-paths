...
```BASH
docker run --rm -v "$PWD:/work" -w /work arnauddegrandmaison440/sme2-learning-path:sme2-environment-v1 make sme2_matmul_intr
```

Then execute ``sme2_matmul_intr`` on the FVP:

```BASH
docker run --rm -v "$PWD:/work" -w /work arnauddegrandmaison440/sme2-learning-path:sme2-environment-v1 ./run-fvp.sh sme2_matmul_intr
```
